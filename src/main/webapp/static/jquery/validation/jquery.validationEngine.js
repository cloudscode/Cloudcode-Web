	...
	var methods = {
		...
		attach : function(userOptions) {
			...
			if (options.binded) {
				...
				form
						.find("[" + options.validateAttribute
								+ "*=validate][type=checkbox],["
								+ options.validateAttribute
								+ "*=validate][type=radio]").bind("click",
								methods._onFieldEvent);
				form.find("[" + options.validateAttribute
		...
		detach : function() {
			...
			form.find("[" + options.validateAttribute + "*=validate]")
					.not("[type=checkbox]").die(options.validationEventTrigger,
							methods._onFieldEvent);
			form.find("[" + options.validateAttribute
					+ "*=validate][type=checkbox]").die("click",
					methods._onFieldEvent);
		...
		_getErrorMessage : function(form, field, rule, rules, i, options,
			...
			if (rule == "custom") {
				var custom_validation_type_index = jQuery.inArray(rule, rules)
						+ 1;
			...
			// Call the original validation method. If we are dealing with
			// dates, also pass the form
			var errorMsg;
			...
			if (errorMsg != undefined) {
				var custom_message = methods._getCustomErrorMessage($(field),
						element_classes_array, beforeChangeRule, options);
				if (custom_message)
					...
					success : function(json) {
						...
						if (errorField.length == 1) {
							...
							if (!status) {
								...
								methods._showPrompt(errorField, msg, "", true,
										options);
							} else {
		...
		_buildPrompt : function(field, promptText, type, ajaxed, options) {
			// hhupdate
			if(field.attr('type')=='check' || field.attr('type')=='checkbox' || field.attr('type')=='radio'){
				field = field.parent('span');
			}
			if(field.is('select')){
				field = field.parent('div');
			}
			field.css("border", "1px solid #cd0a0a");
			...
			prompt.addClass("parentForm"
					+ methods._getClassName(field.parents('form').attr("id")));
		...
		_closePrompt : function(field) {
			...
			if(field.attr('type')=='check' || field.attr('type')=='checkbox' || field.attr('type')=='radio'){
				...
				field.css("border", "1px solid #B5B8C8");
			}
		...
		_condRequired : function(field, rules, i, options) {
			...
			for (idx = (i + 1); idx < rules.length; idx++) {
				 ...
				 * default value; so we won't replicate that here
				 */
